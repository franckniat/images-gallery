...
watch(feature, () => {
  fetchImages(1, feature.value);
});

const fetchImages = async (page: number, collection: string) => {
  loading.value = true;
...
const handlePageChange = (page: number) => {
  ...
  fetchImages(page, feature.value as string);
};

onMounted(() => {
  const page = parseInt(route.query.page as string) || 1;
  currentPage.value = page;
  fetchImages(page, feature.value);
});
